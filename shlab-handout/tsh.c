...
void eval(char *cmdline)
{
    ...
    sigset_t mask_all, mask_fork, prev_fork, empty_set;
    sigfillset(&mask_all);
    sigemptyset(&mask_fork);
    sigemptyset(&empty_set);
    sigaddset(&mask_fork, SIGCHLD);
    sigaddset(&mask_fork, SIGUSR1);


    bg = parseline(cmdline, argv);
    // Do nothing if user input an empty line
    if (argv[0] == NULL)
        return;
    // Return if user request a built-in command
    if (builtin_cmd(argv))
        return;

    sigprocmask(SIG_BLOCK, &mask_fork, &prev_fork);
    pid = fork();
    if (pid == 0)
    {
        setpgid(0, 0);
        ...
        // Wait for SIGUSR1 after job successfully created
    ...
    {
        kill(pid, SIGINT);//job is not added, kill the child process
        return;
    }
    sigprocmask(SIG_SETMASK, &prev_fork, NULL);
    // char* strs[] = {"jobs", NULL};
    // builtin_cmd(strs);
    if (bg)
    {
        //inform user the program running in background
...
void sigtstp_handler(int sig)
{
    pid_t fg_gpid = fgpid(jobs);
    if (fg_gpid) {
        getjobpid(jobs, fg_gpid)->state = ST;
